...
void find_head(llist *ls, int arr_popped[]){
...
void find_tail(llist *ls, int arr_popped[]){
...
void merge(int line1, int line2, llist arr_ll[], int arr_pos[], int arr_line_pos[], int arr_popped[]){
    ...
    if(arr_ll[arr_line_pos[line1]].length <= arr_ll[arr_line_pos[line2]].length){
        ...
        for(p = 0; p < arr_ll[arr_line_pos[line1]].length; p++){
            //printf("%d %d\n", __LINE__, arr_ll[arr_line_pos[line1]].length);
...
/*t_node *merge_tree(t_node *max_root1, t_node *max_root2, int arr_popped[]){
    if(max_root1 != (t_node*)NULL){
        if(max_root2 != (t_node*)NULL){
            if(arr_popped[max_root1 -> value] == 0){//If already popped
                max_root1 = merge_tree(max_root1 -> left, max_root1 -> right, arr_popped);
            }
            if(arr_popped[max_root2 -> value] == 0){//If already popped
                max_root2 = merge_tree(max_root2 -> left, max_root2 -> right, arr_popped);
            }
        }
    }
    if(max_root1 != (t_node*)NULL){
        if(max_root2 != (t_node*)NULL){
            if(max_root1 -> value >= max_root2 -> value){//把line2合併到line1
                if(max_root1 -> left == (t_node*)NULL){
                    max_root1 -> left = max_root2;
                }
                else{
                    max_root1 -> right = merge_tree(max_root1 -> right, max_root2, arr_popped);
                    if(max_root1 -> left -> dist < max_root1 -> right -> dist){
                        t_node *no;
                        no = max_root1 -> left;
                        max_root1 -> left = max_root1 -> right;
                        max_root1 -> right = no;
                    }
                    max_root1 -> dist = max_root1 -> right -> dist + 1;
                }
                return max_root1;
            }
            else{//把line1合併到line2
                if(max_root2 -> left == (t_node*)NULL){
                    max_root2 -> left = max_root1;
                }
                else{
                    max_root2 -> right = merge_tree(max_root2 -> right, max_root1, arr_popped);
                    if(max_root2 -> left -> dist < max_root2 -> right -> dist){
                        t_node *no;
                        no = max_root2 -> left;
                        max_root2 -> left = max_root2 -> right;
                        max_root2 -> right = no;
                    }
                    max_root2 -> dist = max_root2 -> right -> dist + 1;
                }
                return max_root2;
            }
        }
        else{//build your tree
            return max_root1;
        }
    }
    else{
...
int main(){
    ...
    while(t--){
        ...
        while(j < n){
            // bool through = true;
            // while(j < n  && through == true){
                /*printf("%d %d \n", j, id_op);
                if(arr_pos[1]!= -1){
                    printf("%s %d\n","This is the true line position of 1:", arr_line_pos[arr_pos[1]]);
                }
                if(arr_pos[2]!= -1){
                    printf("%s %d\n","This is the true line position of 2:", arr_line_pos[arr_pos[2]]);
                }
                if(arr_pos[1]!= -1 && arr_line_pos[arr_pos[1]] != -1){
                    printf("%s %d\n","This is the true length of line position of 1:", arr_ll[arr_line_pos[arr_pos[1]]].length);
                }*/
                ...
                if(arr_pos[arr_pair[j]] != -1){//已經存進去了
                    if(arr_pair[j] == arr_ll[arr_pos[arr_pair[j]]].head -> value){//head
                        ...
                        arr_ll[arr_pos[arr_pair[j]]].length--;
                        t_or_f[j] = true;
                        j++;
                    }
                    else{
                    ...
                    if(arr_op[id_op][0] == 0){//push
                        ...
                        if(arr_ll[arr_line_pos[arr_op[id_op][1]]].length <= arr_ll[arr_line_pos[arr_op[id_op][2]]].length){//跟著linkedlist的位置去存
                            ...
                            arr_lines[arr_line_pos[arr_op[id_op][1]]].max_root = (t_node*)NULL;
                        }
                        else{
                            ...
                            arr_lines[arr_line_pos[arr_op[id_op][2]]].max_root = (t_node*)NULL;
                        }
                        //printf("%d\n", __LINE__);
